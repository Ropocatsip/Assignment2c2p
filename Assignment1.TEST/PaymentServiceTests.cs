...
public class PaymentServiceTests
{
    ...
    [Fact]
    public async Task Pay_WhenOrderAlreadyApproved_ThrowsBadHttpRequestException()
    {
        var request = CreateValidPaymentRequest("ORD-DUPLICATE", 100.00m);
        _mockPaymentRepository
            .Setup(r => r.HasApprovedTransactionAsync("ORD-DUPLICATE", It.IsAny<CancellationToken>()))
            .ReturnsAsync(true);

        var exception = await Assert.ThrowsAsync<BadHttpRequestException>(() => _paymentService.PayAsync(request));
        ...
        _mockPaymentRepository.Verify(
            r => r.SaveTransactionAsync(It.IsAny<PaymentRequest>(), It.IsAny<PaymentResponse>(), It.IsAny<CancellationToken>()),
            Times.Never
        );
    }

    [Fact]
    public async Task Pay_WhenAmountHasZeroCents_ReturnsApprovedResponseAndSavesTransaction()
    {
        var request = CreateValidPaymentRequest("ORD-APPROVED", 100.00m);
        _mockPaymentRepository
            .Setup(r => r.HasApprovedTransactionAsync("ORD-APPROVED", It.IsAny<CancellationToken>()))
            .ReturnsAsync(false);
    ...
    [Fact]
    public async Task Pay_WhenAmountHasNonZeroCents_ReturnsDeclinedResponseWithCentsCode()
    {
        var request = CreateValidPaymentRequest("ORD-DECLINED-50", 100.50m);
        _mockPaymentRepository
            .Setup(r => r.HasApprovedTransactionAsync("ORD-DECLINED-50", It.IsAny<CancellationToken>()))
            .ReturnsAsync(false);
    ...
    [Fact]
    public async Task Pay_WhenAmountHas99Cents_ReturnsDeclinedResponseWithCode99()
    {
        var request = CreateValidPaymentRequest("ORD-DECLINED-99", 25.99m);
        _mockPaymentRepository
            .Setup(r => r.HasApprovedTransactionAsync("ORD-DECLINED-99", It.IsAny<CancellationToken>()))
            .ReturnsAsync(false);
    ...
    [Fact]
    public async Task Pay_EncryptsCardNumberAndCvvBeforeSaving()
    {
        var request = CreateValidPaymentRequest("ORD-ENCRYPT-CHECK", 50.00m);
        _mockPaymentRepository
            .Setup(r => r.HasApprovedTransactionAsync("ORD-ENCRYPT-CHECK", It.IsAny<CancellationToken>()))
            .ReturnsAsync(false);

        await _paymentService.PayAsync(request);
